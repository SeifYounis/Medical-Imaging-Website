...
def gettable(DATABASE_URL,tablename,assessment=None):
	...
	query = "SELECT * FROM "+tablename+" WHERE DATE_TRUNC('day', answer_date) = '2022-03-25';"
	...
	# 'assessment' column possible entries: training, testing1, testing2, rating, 2AFC
...
def by(name,func,x):  # My own version of the R by function.
	...
	for lvl in lvls:
		try:
			...
			sys.stderr.write('* function '+str(func)+' failed in by() for value %s\n'%str(lvl));
			sys.stderr.flush()
			pass
	return(rtn)
...
#import sys; sys.argv="thisscript results 2AFC".split()

if __name__ == "__main__":
#if True:
	# get script arguments
	dbtable="results"  #  sys.argv[1]   # This is probably "results"
	ClassOrSelfguided=sys.argv[1]   # This is probably "results"
	partno=sys.argv[2]
	

	if ClassOrSelfguided=="Self-guided":  # Temporarily we do this.
		userNAME=sys.argv[3]
		studentID=sys.argv[4]
		sessionID=sys.argv[5]		
		sys.stdout.write('<html>\n<head><title>The self-guided results</title><meta charset="UTF-8"></head>\n<body> \n Placeholder Text for part %s, user %s, student id %s, session %s\n</body>\n</html>\n'% (partno,userNAME,studentID,sessionID))
		sys.exit()
		
	
	#DATABASE_URL = "postgres://pmfelvdmtnoyix:0f5df0325b7a23b2c22fea6a30f4a5bdba7f469e966077e7a9e71b43f09f3018@ec2-54-205-232-84.compute-1.amazonaws.com:5432/d8rnaofcbn3hpq"  # Seif's old database

	DATABASE_URL="postgres://dbxfwqesrovtjh:ad7dfcf5e2fe7032a09f8d973e853010e8b1117d690245b8d695f9a10c28e5c4@ec2-54-173-77-184.compute-1.amazonaws.com:5432/db0d3t8a6c1o7d"  # Frank's database at Heroku.

	if not 'responsesraw' in dir():
		#try:
		#	with open('pickledDB.pkl','r') as fp:
		#		responsesraw=load(fp)
		#except:
		responsesraw=gettable(DATABASE_URL,dbtable)  # Read in the database.
		#	with open('pickledDB.pkl','r') as fp:
		
	responses=copy.deepcopy(responsesraw)
	
	if not len(responses):
		sys.stderr.write("Empty Table\n"); sys.exit(3);

	#Points table:  The part/assessment/task name: (0, TP points, TN points)
	pointstable={ "training":(0,0,0), "testing":(0,1,1), "testing1":(0,1,1),
					  "testing2":(0,10,1), "rating":(0,0,0), "2AFC":(0,0,0)}

	
	# Add  truth and score columns
	#responses=mlab.rec_append_fields(responses,["truth","score",'case'],
	datestr=[ str(i.answer_date.date()) for i in responses]
	responses=rec.append_fields(responses,["truth","score",'case','userid'],
               (sigim3(responses),score(responses.answer),
                casename(responses.prompt_image),
				np.array(responses.username+datestr+responses.student_id,
						 dtype=np.str_)),	asrecarray=True)
	#userid is a rather unique string for each user/session/day, etc.

	trainparty=responses[responses.assessment=='training']
	trprevs=bydict("userid",getprev,trainparty) #What was training prev for users?
	...
	if True:
		...
		for i in np.unique(party.userid):
			if i not in trprevs: trprevs[i]=None
			if i not in groupNumbers: groupNumbers[i]=0

		# ... we are getting some dupes ... keep only unique trials.
		kys=party.userid+party.prompt_image
		party=party[ np.unique(kys,return_index=True)[1]]
		...
		def sensfps(x):  # Calculate sensitivity and spec
			...
			try:
				...
				sys.stderr.write('* failed to assign points for userid '+x.userid[0]+'\n')
				sys.stderr.flush()
				scr=np.nan
			npres=np.sum(x.truth)
			result=(fpf,tpf,lmda,lmda/(lmda+1.),tnN,tpN, scr,-scr,
			    groupNumbers[x[0].userid],x[0].username,(tpf+1.-fpf)/2., (tpf+fpf)/2.,
					  npres,len(x)-npres, fpfSD,tnNSD,tpfSD,tpNSD,trprevs[x[0].userid],
					x[0].userid)
		...
		if "2AFC" not in partno:
			...
			z=recify(by("userid",sensfps,party)) # Calculate Se,sp for each user
			z.sort(order='negscore') # Sort by score.

			zdict=mkdict('userid',z)
		...
		def rocc(x):  # Calculate roc curve
			...
			return(np.array([(ROC.rocxy(sigsc,abssc),a, 
			                  -a+np.random.random()*1e-6, a/(1.-a),
									groupNumbers[x[0].userid], x[0].username,snr,psnr, 
									np.sqrt(vara),np.sqrt(uvara),x[0].userid )], 
			           ...
			           ('SNR',float),('pSNR',float),('auc-SD',float),
							   ('auc-uSD',float),('userid',x.userid.dtype)]))

		if "2AFC" not in partno:
			# The variable zroc contains all the ROC information.
			zroc=recify(by("userid",rocc,party)) # Calculate ROC for each user
			#zroc.sort(order='negauc')  #For some reason this no worky.
			zroc=zroc[np.argsort(zroc.negauc)] #this works 

			zrocdict=mkdict("userid",zroc)
			rddata=bydict("userid",rddatafunc,party) # Make structure of all scores
			try:
			...
			try: 
				...
				for i in party:
					scordict[i.userid][i.case]=i.score
		...
		def scoresnstuff(x):
			...
			readers=np.unique(party.userid)
...
#			pp.axis([-.005,1.005,-.005,1.005])
		...
		def addBinPoints(ax,toplot=None,groups=None):
			...
			for x in toplot:
				...
				if groups: 
					try:
						pch=grps[groups[x['userid']]%len(grps)]
						label="Group "+str(groups[x['userid']])
					except:
						continue
				stor,=ax.plot(x['fpf'],x['tpf'],pch,label=label,ms=7)
				textify(ax,x)
				try:  # readers in group 0 have no training prevalence group.
					if groups[x['userid']]:	legdict[pch]=(stor , label)
				except:
					continue
			if groups:
		...
		def correlation_plot():
			...
			if True:
				...
				youd=[zdict[i]['Youden'] for i in zroc['userid']]
...
#				hdr+=tblntryf(i['auc'])+tblntryMT
				...
				if i['userid'] in zdict:
					youd=zdict[i['userid']]['Youden']
		...
		def htmlhdr(extra=""):
			hdr='<html>\n<head><title>Results Part  %s %s</title><meta charset="UTF-8"></head>\n<body>\n' % (partno,extra)
			return(hdr)

		def dumpcsvdata(filename='tmp'):
			# This function needs to be REWRITTEN.  It is not up to date.
			# First we have to figure out what we are dumping.
			# Which readers have usable data?
			lusers=np.unique(party.userid)
...
#			if True: 
		...
		# This is the rating part with testing
		if 'testing' in partno:  # and partstbl.interfaceType[partno-1]=='Binary':
			...
			try:				# Mark points by groups in the training part.
    ...
    <TD align="center"><B><font size=5>  Prevalence  </font></B></TD>
			...
			try:
				...
				p1groups=bydict('group', meanprev,z)  # group by training group.
				
				for i in p1groups:
					if not p1groups[i][0]: continue #skip readers w/ no training prev.
					if not np.isfinite(p1groups[i][0]): continue
					htmlc.append('<TR>')
					try:
    ...
    <TD align="center"><B><font size=5>  Mean Y=(Sens+Spec)/2 </font></B></TD>
		...
		if partno=='Skip' :  # We just reuse the 
			...
			try:
				...
				# Recalculate part 2
				z2=recify(by("userid",sensfps,party2))
				z2dict=mkdict('userid',z2)
...
<TD align="center"><B><font size=5> Std Dev Y  </font></B></TD>
			...
			try:
				...
				for parti in partses:
					try:
						...
						zner=recify(by("userid",sensfps,party) )
						allz[parti]=mkdict('userid',zner)
						zner=recify(by("userid",rocc,party))
						allzroc[parti]=mkdict("userid",zner)
					except: pass
				rdrs=allz[4].keys()
				for rd in rdrs:
					try:
		...
		if partno=='2AFC':
			...
			def auc2afc(x):
				...
				rtn=np.array( [(auc,-auc,sdauc,x.username[0],x.userid[0])],
 					dtype=[ ('auc',float),('neg',float),('sdauc',float),
						('username',x.username.dtype),('userid',x.userid.dtype) ])
				rtn=rtn.view(np.recarray)
				return(rtn)

			zafc=recify(by("userid",auc2afc,party)) # Calculate Se,sp for each user
			zafc.sort(order='neg')
...
if 'devel' in dir():
	open('Results/testing/testoutput.html','w').write(scriptoutput)
else:
